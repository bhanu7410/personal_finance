    ...
    const activeParentStat = selectedCategoryId
        ? stats.find(
              (s) =>
                  ...
                  s.subcategories?.some((sub) => sub.id === selectedCategoryId),
          )
        : null;
    ...
    const renderBarAndLegend = (
        ...
        return (
            <div className="w-full space-y-4">
                {/* The Actual Bar */}
                <div
                    className={`relative flex gap-0.5 overflow-hidden rounded-2xl bg-gray-100 p-1 shadow-inner ${isSub ? "h-8" : "h-12"}`}
                >
                    {items.map((stat) => {
                        // For parent bar, it's selected if its id matches OR if one of its subcategories matches
                        const isSelected =
                            selectedCategoryId === stat.id ||
                            (!isSub &&
                                stat.subcategories?.some(
                        ...
                        return (
                                ...
                                onClick={() =>
                                    onSelectCategory(
                                        isSelected &&
                                            selectedCategoryId === stat.id
                                            ? isSub
                                                ? activeParentStat?.id || null
                                                : null
                                            : stat.id,
                                    )
                                }
                                className={`group relative flex h-full cursor-pointer items-center justify-center overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] first:rounded-l-xl last:rounded-r-xl ${
                            ...
                            >
                                <span
                                    className={`px-3 font-black whitespace-nowrap text-white transition-all duration-300 ${isHovered || isSelected ? "translate-y-0 opacity-100" : "translate-y-1 opacity-0"} ${isSub ? "text-[8px]" : "text-[10px]"}`}
                                >
                    ...
                    {items.map((stat) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                />
                                <span
                                    className={`leading-none font-bold ${isSub ? "text-[9px]" : "text-[10px]"}`}
                                    style={{ color: "inherit" }}
                                >
                                    {stat.icon} {stat.name}
                                </span>
            ...
            <div className="flex items-center justify-between">
                ...
                {selectedCategoryId && (
                    <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-bold text-orange-600 transition-all hover:text-orange-700 active:scale-95"
                    >
                        <X className="h-3 w-3" />
                        Clear Filter
                    </button>
