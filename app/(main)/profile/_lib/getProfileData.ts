...
export async function getProfileData(days: number | null) {
    ...
    const balanceTrend = await getDashboardChartData(
        userId,
        financialSummary.realized.currentCashBalance,
    );

    // 5. Friend Balances (Who owes me / who I owe)
    const [lentSplits, borrowedSplits, paymentsSent, paymentsReceived] =
        await Promise.all([
    ...
    const friendLedger = new Map<
        string,
        { id: string; name: string; netBalance: number }
    >();
    ...
    lentSplits.forEach(
        (s) =>
            (getFriend(s.userId, s.user.name || "").netBalance +=
                s.amount.toNumber()),
    );
    borrowedSplits.forEach(
        (s) =>
            (getFriend(
                s.transaction.payerId,
                s.transaction.payer.name || "",
            ).netBalance -= s.amount.toNumber()),
    );
    paymentsReceived.forEach(
        (p) =>
            (getFriend(p.senderId, p.sender.name || "").netBalance -=
                p.amount.toNumber()),
    );
    paymentsSent.forEach(
        (p) =>
            (getFriend(p.receiverId, p.receiver.name || "").netBalance +=
                p.amount.toNumber()),
    );
    ...
    const groupsInfo = groups
        .map((group) => {
            ...
            let lastActivityDate = group.createdAt.getTime();

            group.transactions.forEach((t) => {
                totalSpend += t.amount.toNumber();
                const tTime = t.date.getTime();
                if (tTime > lastActivityDate) {
                    lastActivityDate = tTime;
                }
                t.splits.forEach((s) => {
                    if (s.userId === userId) {
                        userContribution += s.amount.toNumber();
                    }
                });
            });
            return {
    ...
    insights.noOfGroups = groups.length;
